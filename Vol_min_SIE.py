...
##    mflow_LOCA = min(mflow_noBloq, mflow_Bloq)
    ...
    if p_prim >= 2*p_cont:
        mflow_LOCA = A_LOCA*(K_bloq*p_prim*rhog*(2/(K_bloq+1))**exp_bloq)**0.5
    elif p_prim < 2*p_cont:
        mflow_LOCA = A_LOCA*(2*(p_prim - p_cont)*rhog/K_LOCA)**0.5
...
Vol_l_SIE_nom_v = [38, 39, 40]

matriz_niveles = []

for Vol_l_SIE_nom in Vol_l_SIE_nom_v:
    print(30*'-')
    print('Inicia para {} m3'.format(Vol_l_SIE_nom))
    
    ##Volumen total del acumulador SIE: 69 m3
    Vol_tot_SIE = 69 #m3

    ##Volumen N2 SIE: 19 m3
    Vol_N2_nom = Vol_tot_SIE - Vol_l_SIE_nom #m3

    nivel_l_SIE_nom = Vol_l_SIE_nom/Vol_tot_SIE
    ...
    if t_sim > 2*3600:
        sim_largo_plazo = True
    else:
        sim_largo_plazo = False
    ...
    trip_SIE   = False
    trip_LOHS  = False
    t_LOHS     = None # Momento en el que se produce el LOHS (colocar "None" en caso de No LOHS)
    trip_LOCA  = False
    t_LOCA     = 100 # Momento al que se produce el LOCA (colocar "None" en caso de No LOCA)
    trip_SCRAM = False
    t_SCRAM_manual = 100.1 # Demanda manual de SCRAM por tiempo
    ...
    Vol_l_SIE     = Vol_l_SIE_nom
    nivel_prim    = (Vol_tot_prim - Vol_g_prim)/Vol_tot_prim
    nivel_l_SIE   = nivel_l_SIE_nom
    ...
    ##            gamma     = 1   # expansión isotérmica
                ...
                Vol_l_SIE   = Vol_tot_SIE - Vol_N2_1
                nivel_l_SIE = Vol_l_SIE/Vol_tot_SIE
...
##        
##        nivel_GV_v.append(r)
##        nivel_l_SIE_v.append(nivel_l_SIE)
##
##        Vol_l_SIE_v.append(Vol_l_SIE)
##        Vol_N2_v.append(Vol_N2_1)
...
##    nivel_l_SIE_v  = np.array(nivel_l_SIE_v)
##
##    Vol_l_SIE_v    = np.array(Vol_l_SIE_v)
##    Vol_N2_v       = np.array(Vol_N2_v)

    matriz_niveles.append(nivel_prim_v)
    print('Finaliza para {} m3'.format(Vol_l_SIE_nom))
    print(30*'-')
...
####plt.plot(t_v, p_sup_SECR_v/1e6,  '--', label = 'demanda de SECR por alta p ({:0.1f} MPa)'.format(p_umbral_sup_SECR/1e6))
####plt.plot(t_v, p_sup_SCRAM_v/1e6, '--', label = 'demanda de SCRAM por alta p ({:0.1f} MPa)'.format(p_umbral_sup_SCRAM/1e6))
##plt.plot(t_v, p_inf_SCRAM_v/1e6, '--', label = 'demanda de SCRAM por baja p ({:0.1f} MPa)'.format(p_umbral_inf_SCRAM/1e6))
##plt.plot(t_v, p_inf_SECR_v/1e6,  '--', label = 'demanda de SECR por baja p ({:0.1f} MPa)'.format(p_umbral_inf_SECR/1e6))
##plt.plot(t_v, p_SIE_v/1e6,       '--', label = 'demanda de SIE por baja p ({:0.1f} MPa)'.format(p_umbral_SIE/1e6))
...
##plt.figure(2)
##plt.grid() 
##plt.plot(t_v, Q_gen_v/1e6,    label = 'Q generado')
##plt.plot(t_v, Q_transf_v/1e6,'--', label = 'Q_transferido')
##plt.plot(t_v, Q_comb_v/1e6,   label = 'Q combustible')
##plt.legend(loc="best", shadow=True, fontsize = 'large')
##if sim_largo_plazo:
##    plt.xlabel('Tiempo (h)', fontsize = 'x-large')
##else:
...
##plt.figure(3)
##plt.grid()
##plt.plot(t_v, T_prim_v-273.15, label = 'T primario')
##plt.plot(t_v, T_comb_v-273.15, label = 'T combustible')
##plt.plot(t_v, T_estr_v-273.15,'--', label = 'T estructuras')
##plt.plot(t_v, T_GV_v-273.15,   label = 'T en GV')
##plt.legend(loc="best", shadow=True, fontsize = 'large')
##if sim_largo_plazo:
##    plt.xlabel('Tiempo (h)', fontsize = 'x-large')
##else:
##    plt.xlabel('Tiempo (s)', fontsize = 'x-large')
##plt.ylabel('Temperatura (ºC)', fontsize = 'x-large')
##
##plt.figure(4)
##plt.grid()
##plt.plot(t_v, mflow_LOCA_v, label = 'caudal másico de LOCA')
##plt.plot(t_v, mflow_SIE_v, label = 'caudal másico de SIE')
##plt.legend(loc="best", shadow=True, fontsize = 'large')
##if sim_largo_plazo:
##    plt.xlabel('Tiempo (h)', fontsize = 'x-large')
##else:
##    plt.xlabel('Tiempo (s)', fontsize = 'x-large')
##plt.ylabel('caudal másico (Kg/s)', fontsize = 'x-large')

plt.figure(1)
plt.grid()
for i,v in enumerate(Vol_l_SIE_nom_v): 
    plt.plot(t_v, matriz_niveles[i], label = 'volumen inicial SIE = {} m3'.format(v))
...
##plt.figure(8)
##plt.grid()
##plt.plot(t_v, nivel_l_SIE_v, label = 'nivel de liquido en el SIE')
##plt.legend(loc="best", shadow=True, fontsize = 'large')
##if sim_largo_plazo:
##    plt.xlabel('Tiempo (h)', fontsize = 'x-large')
##else:
...
##plt.figure(9)
##plt.grid()
##plt.plot(t_v, Vol_l_SIE_v, label = 'volumen de liquido en el SIE')
##plt.plot(t_v, Vol_N2_v,    label = 'volumen de N2 en el SIE')
##plt.legend(loc="best", shadow=True, fontsize = 'large')
##if sim_largo_plazo:
##    plt.xlabel('Tiempo (h)', fontsize = 'x-large')
##else:
##    plt.xlabel('Tiempo (s)', fontsize = 'x-large')
##plt.ylabel('volumen (m3)', fontsize = 'x-large')
